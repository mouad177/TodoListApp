   ...
   <>
      <div className="bg-white relative p-5 rounded-xl w-full md:w-[50%] gap-6 flex flex-col items-center justify-center  ">
        ...
        <ul className="text-gray-600 flex flex-row-reverse items-center gap-[25px] cursor-default">
          ...
          <li onClick={()=>{setSelectBtnType('all')}}  className={`border ${selectBtnType=='all'?'bg-blue-400 text-white':"hover:bg-gray-200 duration-200"} border-1 border-gray-300 rounded-lg p-1 font-medium`}>الكل</li>
           <li onClick={()=>{setSelectBtnType('non-completed')}} className={`border ${selectBtnType=='non-completed'?'bg-blue-400 text-white':"hover:bg-gray-200 duration-200"} border-1 border-gray-300 rounded-lg p-1 font-medium`}>
            غير منجز
          </li>
                    <li onClick={()=>{setSelectBtnType('completed')}}  className={`border ${selectBtnType=='completed'?'bg-blue-400 text-white':"hover:bg-gray-200 duration-200"} border-1 border-gray-300 rounded-lg  p-1 font-medium`}>منجز</li>

        </ul>
<div className="h-2 flex items-center ">
                             <button onClick={()=>{setShowDeleteAllModal(true)}} className={`left-8 ${tasks.length>1?'flex':'hidden'}  m-3 gap-2 top-28  items-center bg-red-500 hover:bg-red-700 duration-200 hover:scale-105 focus:scale-95 text-xs  text-white p-1 rounded-sm`}>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 56 56"><path fill="currentColor" d="M13.785 49.574h28.453c4.899 0 7.336-2.437 7.336-7.265V13.69c0-4.828-2.437-7.265-7.336-7.265H13.785c-4.875 0-7.36 2.414-7.36 7.265v28.62c0 4.851 2.485 7.265 7.36 7.265m5.906-11.203c-1.148 0-2.109-.937-2.109-2.086c0-.539.234-1.054.656-1.476l6.797-6.844l-6.797-6.82c-.422-.422-.656-.938-.656-1.477c0-1.172.96-2.133 2.11-2.133c.562 0 1.054.211 1.476.633l6.844 6.844l6.843-6.844a1.973 1.973 0 0 1 1.454-.633a2.14 2.14 0 0 1 2.132 2.133c0 .54-.234 1.055-.656 1.477l-6.82 6.82l6.82 6.844c.422.422.656.937.656 1.476c0 1.149-.96 2.086-2.133 2.086c-.538 0-1.054-.21-1.43-.586l-6.866-6.89l-6.868 6.89a2.036 2.036 0 0 1-1.453.586"/></svg>
            حذف الكل
          </button>
</div>
        <div   className={`max-h-[50vh] p-2 w-full overflow-y-auto flex gap-2 flex-col`}>
        {TaskList}
