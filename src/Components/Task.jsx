  ...
  return (
    <>
      <div  className="w-full flex flex-row-reverse gap-5 items-center p-3 rounded-xl justify-between bg-linear-to-r from-blue-800 to-blue-400">
        <div className="flex flex-row-reverse items-center gap-3 w-3/4  ">
          <button
          ...
          >
            <h2 className={`${task.isCompleted ? "line-through opacity-85 text-gray-300" : "" } text-white text-right text-3xl    ${task.details.split('').length>20?'wrap-anywhere':'break-words'} `}>{task.title}</h2>

          
              <p  ref={ref} dir="rtl" className={`text-white ${isReadMore?'line-clamp-none ':'line-clamp-2 '} text-right text-sm  ${task.isCompleted ? "line-through opacity-85 text-gray-300 " : "" }
               ${task.details.split('').length>20?'wrap-anywhere':'break-words'}`}> 
               {task.details}
